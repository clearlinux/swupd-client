...
int list_installable_bundles()
{
	...
	MoM = load_mom(current_version, false, mix_exists, NULL, false);
	if (!MoM) {
		return EMOM_LOAD;
	}
...
static int load_bundle_manifest(const char *bundle_name, struct list *subs, int version, struct manifest **submanifest)
{
	...
	*submanifest = NULL;
	...
	mom = load_mom(version, false, false, NULL, false);
	if (!mom) {
		return EMOM_LOAD;
	}
...
int show_included_bundles(char *bundle_name)
{
	...
	mom = load_mom(current_version, false, false, NULL, false);
	if (!mom) {
...
int show_bundle_reqd_by(const char *bundle_name, bool server)
{
	...
	current_manifest = load_mom(version, server, false, NULL, false);
	if (!current_manifest) {
...
int remove_bundles(char **bundles)
{
	...
	for (; *bundles; ++bundles, total++) {
		...
		*/
		...
		current_mom = load_mom(current_version, false, mix_exists, NULL, false);
		if (!current_mom) {
			fprintf(stderr, "Unable to download/verify %d Manifest.MoM\n", current_version);
			ret = EMOM_LOAD;
			bad++;
