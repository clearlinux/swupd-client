...
export SWUPD_OK=0
export SWUPD_NO=1
export SWUPD_COULDNT_LOAD_MOM=2  # MoM cannot be loaded into memory (this could imply network issue)
export SWUPD_COULDNT_REMOVE_FILE=3  # do_staging() couldn't delete a file which must be deleted
export SWUPD_COULDNT_OVERWRITE_DIR=4  # do_staging() couldn't overwrite a directory
export SWUPD_COULDNT_CREATE_DOTFILE=5  # do_staging() couldn't create a dotfile
export SWUPD_RECURSE_MANIFEST=6  # error while recursing a manifest
export SWUPD_LOCK_FILE_FAILED=7  # cannot get the lock
export SWUPD_CURL_INIT_FAILED=8  # cannot initialize curl agent
export SWUPD_INIT_GLOBALS_FAILED=9  # cannot initialize globals
export SWUPD_BUNDLE_NOT_TRACKED=10  # bundle is not tracked on the system
export SWUPD_COULDNT_LOAD_MANIFEST=11  # cannot load manifest into memory
export SWUPD_INVALID_OPTION=12  # invalid command option
export SWUPD_SERVER_CONNECTION_ERROR=13  # no net connection to swupd server
export SWUPD_COULDNT_DOWNLOAD_FILE=14  # file download problem
export SWUPD_COULDNT_CREATE_DIRS=15  # Cannot create required dirs
export SWUPD_CURRENT_VERSION_UNKNOWN=16  # Cannot determine current OS version
export SWUPD_COULDNT_VERIFY_SIGNATURE=17  # Cannot initialize signature verification
export SWUPD_BAD_TIME=18  # System time is bad
export SWUPD_COULDNT_DOWNLOAD_PACK=19  # Pack download failed
export SWUPD_BAD_CERT=20  # unable to verify server SSL certificate
export SWUPD_REQUIRED_BUNDLE_ERROR=21  # a required bundle is missing or was attempted to be removed
export SWUPD_INVALID_BUNDLE=22  # the specified bundle is invalid
export SWUPD_DISK_SPACE_ERROR=23  # not enough disk space left (or it cannot be determined)
export SWUPD_COULDNT_UNTAR_FILE=24  # couldn't untar a file
export SWUPD_PATH_NOT_IN_MANIFEST=25  # the required path is not in any manifest
