...
static enum swupd_code create_wrapper_script(void *data)
{
	...
	fprintf(fp, "#!/bin/bash\n");
	fprintf(fp, "\n");
	fprintf(fp, "export PATH=%s%s%s\n", third_party_bin_path, env_path ? ":" : "", env_path ? env_path : "");
	fprintf(fp, "export LD_LIBRARY_PATH=%s%s%s\n", third_party_ld_path, env_ld_path ? ":" : "", env_ld_path ? env_ld_path : "");
	fprintf(fp, "\n");
	fprintf(fp, "%s \"$@\"\n", binary);

	/* set the file mode to 0755 */
	if (chmod(script, mode) < 0) {
		error("Unable to set permissions for file %s\n", script);
