...
test_setup() {
	...
	key_generation_cmd=$(cat <<-EOM
			...
			openssl x509 -days 600 -req -in "$TEST_NAME/cert_mom.csr" -CA "$TEST_NAME/root.pem" -CAkey "$TEST_NAME/root.key" -set_serial 0x1 -out "$TEST_NAME/cert_mom.pem"
...
@test "SIG026: Swupd update with expired latests certificate" {

	# Swupd update successful if latest certificate is expired, but MoM
